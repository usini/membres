class DolibarrAPI {
  ...
  async getMember(username) {
    ...
    const route = `members?sortfield=t.rowid&sortorder=ASC&limit=1000&sqlfilters=(t.login%3Alike%3A'%25${username}%25')`
    try {
      ...
      this.usersList = data.filter(function (element) {
        return element.datefin >= Date.now() / 1000;
      });
  ...
  async getClient(username) {
    var requestOptions = this.createOptions();
    username = encodeURIComponent(username);
    const route = `thirdparties?sortfield=t.rowid&sortorder=ASC&limit=100&sqlfilters=(t.nom%3Alike%3A'%25${encodeURIComponent(username)}%25')`;
